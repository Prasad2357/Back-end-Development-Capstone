...
def signup(request):
    if request.method == "POST":
        ...
        try:
            user = User.objects.filter(username=username)  # Find user using User.objects.filter method
            if user.exists():  # Check if user already exists
                form = SignUpForm()  # Render the signup.html page with SignUpForm
                return render(request, 'signup.html', {'form': form, 'message': "User already exists."})
            else:
                # Create a new user using the User.objects.create method
            ...
            form = SignUpForm()  # Render the 'signup.html' page with the 'SignUpForm' form
            return render(request, 'signup.html', {'form': form, 'message': "Error occurred."})

    return render(request, 'signup.html', {'form': SignUpForm()})  # Render the 'signup.html' page with the 'SignUpForm' form
